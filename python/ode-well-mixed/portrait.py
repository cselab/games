...
def J_ode(Y, t, beta=1, gamma=0.05):
    ...
    y2dot = -gamma*y2 + 0.5*np.exp(beta*y2)*(np.exp(beta*y1)+np.exp(beta*y2))/(np.exp(beta*y1)+np.exp(beta*y2)+np.exp(beta*y3))**2
    y3dot = -gamma*y3 + 0.7*np.exp(beta*y3)*(np.exp(beta*y1))             /(np.exp(beta*y1)+np.exp(beta*y2)+np.exp(beta*y3))**2
